...
Attempting to pass vectors of 6 integers instead of 6 strings of characters.
'''
...
output_str_all = Dense(6, activation='relu', name='output_str_all')(encoded_str_all)
...
# for iteration in range(1, 60):
    ...
    model.fit(X, y, batch_size=128, nb_epoch=5, callbacks=[checkpoint])
    ...
    for i in range(200):
        ...
        for t in xrange(all_timestep.shape[0]):
            for j, tstep in enumerate(all_timestep[t]):
                x[0, t, j] = tstep
#             for t, char in enumerate(all_timestep):
...
#             X[i, t, j] = tstep
        ...
        # all_timestep = all_timestep[1:] + next_char1
        all_timestep = np.concatenate((all_timestep[1:], \
                                            preds.reshape(1,6)), axis=0)
        ...
        # output_text += str(preds) + '\n'

        tab_chunk = np.concatenate((tab_chunk, preds.reshape(1,6)), axis=0)
        # output_text += next_char1 + next_char2 + next_char3 + next_char4 + \
        #                             next_char5 + next_char6 + '.'
        output_text += str(tab_chunk)
